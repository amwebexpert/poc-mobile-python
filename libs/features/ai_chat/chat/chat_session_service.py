...
class ChatSessionService:
    ...
    def get_all_sessions(self) -> List[ChatSession]:
        ...
        for chat_session_rs in result:
            chat_session = ChatSession(
                id=chat_session_rs[0], iso_created_at=chat_session_rs[1], title=chat_session_rs[2])
            chat_sessions.append(chat_session)
        self.storage_service.close()
        chat_sessions.sort(
            key=lambda session: session.iso_created_at, reverse=True)
    ...
    def save(self, chat_session: ChatSession) -> ChatSession:
        ...
        for chat_item in chat_session.items:
            if chat_item.id == 0:
                chat_item.id = self.add_chat_session_item(
                    chat_session, chat_item)
    ...
    def add_chat_session_item(self, chat_session: ChatSession, chat_item: ChatItem) -> int:
        chat_item.id = self.storage_service.insert(
            table_name="chat_session_items",
            columns=("chat_session_id", "iso_created_at",
                     "description", "role"),
            values=(chat_session.id, chat_item.iso_created_at,
                    chat_item.description, chat_item.role)
        )
        return chat_item.id
    ...
    def load_chat_session_items(self, chat_session: ChatSession) -> None:
        result = self.storage_service.select(
            "chat_session_items", ["*"], f"chat_session_id = {chat_session.id}")
